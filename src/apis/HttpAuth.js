...
export const HttpAuth = {
    ...
    Registro: async ( data ) => {
        try{
            ...
            let consultaRegistro = await fetch(`${url}registrarMedico`,{ method: 'POST', headers: headers, body:JSON.stringify(data)})
    ...
    Loguear: async ( data ) => {
        try{
            const headers = new Headers()
            headers.append('Content-Type', 'application/json');
            let consultaLogin = await fetch(`${url}login`,{ method: 'POST', headers: headers, body:JSON.stringify(data)})
